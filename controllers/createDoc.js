...
export async function createDoc(req, res) {
  ...
  childInfo.forEach(
    (item) => (item.dateOfBirth = modifyDateOfBirth(item.dateOfBirth))
  );
  ...
  for (let dispancer of dispancers) {
    try {
      ...
      switch (req.body.competitonType) {
        case "Чемпионат":
          childInfo.forEach(
            (item) => (item.sportsCategory = modifySportsCategory(item))
          );
          filePath = await generateChampionshipReport(
            dispancer,
            childInfo,
            req.body.year,
            __dirname
          ...
          break;
        case "Ruza Cup":
          filePath = await generateRuzaCupReport(
            dispancer,
            childInfo,
            req.body.year,
            req.body.dateOfCompetition,
            __dirname
          );
          break;
        case "Турнир":
          filePath = await generateOpenChampionshipReport(
            dispancer,
            childInfo,
            req.body.year,
            req.body.dateOfCompetition,
        ...
        case "Золотая рыбка":
          ...
          filePath = await generateGoldenFishReport(
            dispancer,
            childInfo,
            req.body.year,
            req.body.girlsBirthYears,
            req.body.boysBirthYears,
            req.body.stage,
            __dirname
          );
          break;
